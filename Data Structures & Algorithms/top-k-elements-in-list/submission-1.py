class Solution:
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
        ...
        res = []
        for i in range(len(freq) - 1, 0, -1):  # traversing in desc order for len(freq) - 1, since freq will be 8 for nums = 7, 0 indicates end of loop, -1 is desc
            for n in freq[i]:  # Traversing freq array. ith element is the array of nums with freq i
                res.append(n)
                if len(res) == k: # helps us stop the loop when res length becomes k
                    return res
